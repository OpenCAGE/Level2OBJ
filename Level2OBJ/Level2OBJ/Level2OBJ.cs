...
namespace Level2OBJ
{
    public partial class Level2OBJ : Form
    {
        string cinematicToolDLL = "";
        string utilPath = "";

        /* On init, if we are trying to launch to a map, skip GUI */
        public Level2OBJ(string MapToLaunchTo = "")
        {
            Commands cmds = new Commands("G:\\SteamLibrary\\steamapps\\common\\Alien Isolation\\DATA\\ENV\\PRODUCTION\\BSP_TORRENS\\WORLD\\COMMANDS.PAK");

            ShortGuid checksum = new ShortGuid("00-00-00-00");
            EntityHierarchy hierarchy = null;

            for (int i = 0; i < cmds.Entries.Count; i++)
            {
                for (int x = 0; x < cmds.Entries[i].overrides.Count; x++)
                { 
                    hierarchy = cmds.Entries[i].overrides[x].connectedEntity;
                    checksum = cmds.Entries[i].overrides[x].checksum;
                    break;
                }
                if (hierarchy != null) break;
            }

            Console.WriteLine("Checksum: " + checksum.ToByteString());
            Console.WriteLine("Hierarchy: " + hierarchy.GetHierarchyAsString());
            Console.WriteLine("Generated: " + hierarchy.GenerateChecksum());

            return;


            CharacterAccessorySets bruh = new CharacterAccessorySets("G:\\SteamLibrary\\steamapps\\common\\Alien Isolation\\DATA\\ENV\\PRODUCTION\\TECH_RND_HZDLAB\\WORLD\\CharacterAccessorySets.BIN");
            bruh.Save();
           ...
           //Models mdl = new Models("G:\\SteamLibrary\\steamapps\\common\\Alien Isolation\\DATA\\ENV\\PRODUCTION\\TECH_HUB\\RENDERABLE\\LEVEL_MODELS.PAK");
           //mdl.Save();
           //
           //return;

            Level lvl = new Level("G:\\SteamLibrary\\steamapps\\common\\Alien Isolation\\DATA\\ENV\\PRODUCTION\\SOLACE");
            lvl.Save();
            ...
            List<string> content = File.ReadAllLines("enums.txt").ToList();
            List<string> content_s = new List<string>();
            foreach (string s in content)
            {
                if (content_s.Contains(s)) continue;
                content_s.Add(s);
            }
            File.WriteAllLines("enums.csv", content_s);
            return;
        ...
        private void Level2OBJ_Click(object sender, EventArgs e)
        {
        ...
        private void InjectCinematicTools(Level2OBJ mainInst)
        {
